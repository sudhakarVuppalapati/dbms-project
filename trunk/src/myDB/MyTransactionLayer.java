...
public class MyTransactionLayer implements TransactionLayer {
	...
	public boolean abortTransaction(int TID) throws NoSuchTransactionException {
		...
		if (!trans.isActive(TID))
			throw new NoSuchTransactionException("Cannot abort transaction "+TID);
		
		//Write log ahead
		LogRecord logRecord = new LogRecord(TID, LoggedOperation.ABORT_TRANSACTION, null);
		...
		try {
			for (int i = trans.getOpNo(TID) - 1; i >= 0; i--) {
				...
				t = (MyTable)storageLayer.getTableByName(payload.getTableName());
		...
		//Release locks for all tables. Note that enumerations of hashtable are not fail-fast
	...
	public int beginTransaction() {
		...
		LogRecord log = new LogRecord(transID, LoggedOperation.START_TRANSACTION, null);
	...
	public boolean commitTransaction(int TID) throws NoSuchTransactionException {
		...
		if (trans.isActive(TID)) {
			//Write log ahead
			LogRecord log = new LogRecord(TID, LoggedOperation.COMMIT_TRANSACTION, null);
			logStore.writeRecord(log);
		...
		else throw new NoSuchTransactionException("Cannot commit. Invalid transaction ID: "+TID);
	}
	...
	public String explain(int TID, RelationalAlgebraExpression query)
		...
		if (!trans.isActive(TID))
			throw new NoSuchTransactionException("Cannot explain query for transaction: "+TID);
		...
		synchronized (lockedTables) {
			...
			else throw new TableLockedException();
		}
	...
	public void insertRow(int TID, String tableName, Row row)
		...
		if (!trans.isActive(TID))
			throw new NoSuchTransactionException("Cannot insert row. Invalid transaction ID: "+TID);
		...
		//Write log ahead
		LogRecord log = new LogRecord(TID, LoggedOperation.INSERT, new InsertLogPayload(rowID, tableName, row));
		trans.writeOperation(TID, log);
		logStore.writeRecord(log);
	...
	public Operator<? extends Row> query(int TID,
		...
		if (!trans.isActive(TID))
			throw new NoSuchTransactionException("Cannot perform query for transaction: "+TID);
	...
	public void updateRow(int TID, String tableName, Row oldRow, Row newRow)
		...
		if (!trans.isActive(TID))
			throw new NoSuchTransactionException("Cannot update row. Invalid transaction ID: "+TID);
	...
	private boolean testLocks(RelationalAlgebraExpression query, int TID, Map pushTables) {
		...
		if (type == RelationalOperatorType.INPUT) {
			...
			else return (transID.intValue() == TID);
		}
		
		else if (type == RelationalOperatorType.CROSS_PRODUCT) {
			return testLocks(((CrossProduct)query).getLeftInput(), TID, pushTables) & 
			testLocks(((CrossProduct)query).getRightInput(), TID, pushTables);
		}
	...
	private final class TransactionMap {
		...
		synchronized int newTransaction() {
			...
			//logStore.writeRecord(new LogRecord(transNo, LoggedOperation.START_TRANSACTION, null));
			return transNo++;
		}
