...
public class MyRecoveryManager implements RecoveryManager {
	...
	/**
	 * transactions table Maps a TID to the lists of LogRecords corresponding to
	 * that transaction
	 */
	private HashMap<Long, ArrayList<LogRecord>> tt = new HashMap<Long, ArrayList<LogRecord>>();

	/*
	 * a linked list to keep the order of the operations this might be somehow
	 * redundant but for the moment is a decent solution
	 */
	...
	/**
	 * Basic strategy: - Analysis phase - Undo phase
	 * 
	 * This strategy is not the one used by ARIES, but a much simpler one.
	 * Basically, I go through the LogStore and build the tt using the
	 * Logrecords I encounter.
	 * 
	 * When finding an Abort/Commit LogRecord I remove the corresponding TID
	 * from tt as it means that this transaction completed(successfully or not)
	 * and it left the database in a consistent state
	 * 
	 * When reaching the end of the LogStore, the Analysis phase is over. Now tt
	 * should contain only the unfinished transactions that need to be undone.
	 * So, I iterate through the linkedlist and I issue the inverse operation of
	 * each component of the retrieved list.
	 */
	@Override
	public void recover(LogStore logStore) {
		...
		Operator<LogRecord> logRecs = (Operator<LogRecord>) logStore
				.getLogRecords();
		...
		if (logRecs != null) {
			...
			while ((lr = logRecs.next()) != null) {
				 ...
				 * //ArrayList list=tt.remove(); //if(list != null)
				...
				/*
				 * if a normal operation (update, delete, insert) is
				 * encountered, just add it to the list of ops of the
			...
			/*
			 * Redo phase: iterate through the LinkedList and apply the inverse
			 * of each operation
			 */
			for (LogRecord lrec : orderedLogs) {
				...
				if (loggedOp == LoggedOperation.UPDATE) {
					try {
						storageLayer.getTableByName(tableName).updateRow(rId,
								((UpdateLogPayload) payload).getNewRow());
					...
					try {
						((MyTable) storageLayer.getTableByName(tableName))
								.addRow(((InsertLogPayload) payload)
										.getInsertedRow());
					} catch (NoSuchTableException e) {
						e.printStackTrace();
					} catch (SchemaMismatchException e) {
