...
public class SelectionOperator {
	 ...
	 */
	public static Map<String, Column> select(Map<String, Column> data,
			PredicateTreeNode predicate, String relation, IndexLayer indexLayer)
			throws InvalidPredicateException, SchemaMismatchException,
			NoSuchTableException {
		return multipleIndexSelect(data, relation, predicate, indexLayer);
	}

	private static Map<String, Column> multipleIndexSelect(
			Map<String, Column> data, String relations,
		...
		if (relations != null) {
			result = singleBoundedSelect(data, relations, predicate,
					indexLayer, null);
		...
		/**
		 * TODO 1. defer the checking deleted rows until final result retrieval
		 * 2. Improvement1: keep track of deleted rows ID at the first column,
		 * so we dont need to check every column 3. Improvement2: dont need to
		 * create new array. Just set data directly in existing column. To do
		 * that, we need to create type-specific update() method for each Column
		 * (no use of update(int, Object) anymore, except in MyTable) 4.
		 * Improvement3: no need for eraseOldArray();
		 */
		...
		for (Column col : data.values()) {
			...
			if (type == Types.getIntegerType()) {
				...
				/*
				 * if (relations != null) tmpCol = new
				 * MyIntColumn(col.getColumnName(), type, newData); else {
				 */
				...
				/*
				 * if (relations != null) tmpCol = new
				 * MyFloatColumn(col.getColumnName(), type, newData); else {
				 */
				((MyColumn) col).eraseOldArray();
				((MyColumn) col).setData(newData, k);
				/* } */
				...
				/*
				 * if (relations != null) tmpCol = new
				 * MyDoubleColumn(col.getColumnName(), type, newData); else {
				 */
				((MyColumn) col).eraseOldArray();
				((MyColumn) col).setData(newData, k);
				/* } */
				...
				/*
				 * if (relations != null) tmpCol = new
				 * MyLongColumn(col.getColumnName(), type, newData); else {
				 */
				((MyColumn) col).eraseOldArray();
				((MyColumn) col).setData(newData, k);
				/* } */
			...
			// THIS IS REFERENCE, NOT COPY
				...
				for (int i = 0; i < n; i++) {
					...
					if (tmpContent[result[i]] != MyNull.NULLOBJ)
						newData[k++] = tmpContent[result[i]];
				}
				/*
				 * if (relations != null) tmpCol = new
				 * MyObjectColumn(col.getColumnName(), type, newData); else {
				 */
				((MyColumn) col).eraseOldArray();
				((MyColumn) col).setData(newData, k);
				/* } */
			...
			/*
			 * if (relations != null) { colList.add(tmpCol); }
			 */

		}

		// Possible optimization: Creating new hashMap is faster than clearing
		// the existing one
		// Razvan: no need for this
		// Razvan: no need for this
		/*
		 * if (relations != null) { data.clear(); for (Column col : colList) {
		 * data.put(col.getColumnName(), col); } }
		 */
		return data;
	}

	private static int[] boundedScanByConjunct(Map<String, Column> data,
			PredicateTreeNode conjunct, int[] bounds)
	throws NotLeafNodeException, InvalidPredicateException {
		...
		if (op == ComparisonOperator.EQ) {
			if (type == Types.getIntegerType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Integer.MAX_VALUE
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Float.MAX_VALUE
							&& values[bounds[i]] == value)
						tmpResult[matchNo++] = bounds[i];
				}
			} else if (type == Types.getDoubleType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Double.MAX_VALUE
							&& values[bounds[i]] == value)
						tmpResult[matchNo++] = bounds[i];
				}
			} else if (type == Types.getLongType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Long.MAX_VALUE
							&& values[bounds[i]] == value)
						tmpResult[matchNo++] = bounds[i];
				}
			} else {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != null
							&& values[bounds[i]] != MyNull.NULLOBJ
			...
			if (type == Types.getIntegerType()) {
				...
				try {
					...
					for (int i = 0; i < n; i++) {
						if (values[bounds[i]] != null
								&& values[bounds[i]] != MyNull.NULLOBJ
								&& value.compareTo(values[bounds[i]]) >= 0)
							tmpResult[matchNo++] = bounds[i];
					}
		...
		/** Less than or equal */
			...
			if (type == Types.getIntegerType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Integer.MAX_VALUE
							&& values[bounds[i]] != Integer.MIN_VALUE
							&& values[bounds[i]] <= value)
						tmpResult[matchNo++] = bounds[i];
				}
			} else if (type == Types.getFloatType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Float.MAX_VALUE
							&& values[bounds[i]] != Float.MIN_VALUE
							&& values[bounds[i]] <= value)
						tmpResult[matchNo++] = bounds[i];
				}
			} else if (type == Types.getDoubleType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Double.MAX_VALUE
							&& values[bounds[i]] != Double.MIN_VALUE
							&& values[bounds[i]] <= value)
						tmpResult[matchNo++] = bounds[i];
				}
			} else if (type == Types.getLongType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[bounds[i]] != Long.MAX_VALUE
							&& values[bounds[i]] != Long.MIN_VALUE
							&& values[bounds[i]] <= value)
						tmpResult[matchNo++] = bounds[i];
				}
			} else {
	...
	private static int[] singleBoundedSelect(Map<String, Column> data,
			String relation, PredicateTreeNode pTreeNode,
			IndexLayer indexLayer, int[] bounds)
	throws InvalidPredicateException, SchemaMismatchException,
	NoSuchTableException {
		...
		if (pTreeNode.isLeaf()) {
			try {
				...
				indexedSelectByConjunct(indexNames, pTreeNode, indexLayer);
				else
					if (bounds != null)
						return boundedScanByConjunct(data, pTreeNode, bounds);
					else
						return scanByConjunct(data, pTreeNode);
			...
			try {
				...
				if (logicOp == LogicalOperator.AND) {
					if (rightChild.isLeaf()) {
						int[] result = singleBoundedSelect(data, relation,
								rightChild, indexLayer, bounds);
						return singleBoundedSelect(data, relation, leftChild,
								indexLayer, result);
					} else {
						int[] result = singleBoundedSelect(data, relation,
								leftChild, indexLayer, bounds);
						return singleBoundedSelect(data, relation, rightChild,
								indexLayer, result);
					}
				}
				// OR
				else {
					int[] leftResult = singleBoundedSelect(data, relation,
							leftChild, indexLayer, bounds);
					int[] rightResult = singleBoundedSelect(data, relation,
							pTreeNode, indexLayer, bounds);

					// ERROR-PRONE: the following merge assumes that the results
					// are ordered.
	...
	private static int[] boundedSelect(Map<String, Column> data,
			PredicateTreeNode pTreeNode, int[] bounds)
	throws InvalidPredicateException {
		if (pTreeNode.isLeaf()) {
			try {
				if (bounds != null)
	...
	private static int[] indexedSelectByConjunct(String[] indexNames,
			PredicateTreeNode predicate, IndexLayer indexLayer)
	throws NotLeafNodeException, NoSuchIndexException,
	SchemaMismatchException, NoSuchTableException,
	InvalidPredicateException {
		...
		/**
		 * Look for the appropriate index, if we are processing the
		...
		try {
			...
			// TODO Might need to estimate the result size here, determine if
			// it's
			...
			/** 1.2- Greater or equal */
				...
				for (int i = 0; i < n; i++) {
					indexName = indexNames[i];
					if (indexLayer.supportsRangeQueries(indexName))
						return indexLayer.rangeQueryRowIDs(indexName, value,
								null);
				}
			}
			...
			/** 1.3- Less or equal */
				...
				for (int i = 0; i < n; i++) {
					indexName = indexNames[i];
					if (indexLayer.supportsRangeQueries(indexName))
						return indexLayer.rangeQueryRowIDs(indexName, null,
								value);
				}
			}
			...
			/** 1.4- Greater */
				...
				for (int i = 0; i < n; i++) {
					indexName = indexNames[i];
					if (indexLayer.supportsRangeQueries(indexName)) {
						lowerbounds = indexLayer.pointQueryRowIDs(indexName,
								value);
						bounds = indexLayer.rangeQueryRowIDs(indexName, value,
								null);

						// TODO might need to sort two arrays, then retrieve the
						// exclusion
						...
						if (m > 0) {
							...
							// TODO think about the alternatives. This is
							// dangerous !!
							return result;
						} else
							return bounds;
			...
			/** 1.5- Less */
				...
				for (int i = 0; i < n; i++) {
					indexName = indexNames[i];
					if (indexLayer.supportsRangeQueries(indexName)) {
						upperbounds = indexLayer.pointQueryRowIDs(indexName,
								value);
						bounds = indexLayer.rangeQueryRowIDs(indexName, null,
								value);
			...
			/** 1.6- Not equal */
				...
				for (int i = 0; i < n; i++) {
					...
					if (indexLayer.supportsRangeQueries(indexName)) {
						...
						all = indexLayer
						.rangeQueryRowIDs(indexName, null, null);
	...
	private static int[] scanByConjunct(Map<String, Column> data,
			PredicateTreeNode conjunct) throws NotLeafNodeException,
			InvalidPredicateException {
		...
		/** Less than */
			...
			if (type == Types.getIntegerType()) {
				...
				for (int i = 0; i < n; i++) {
					if (values[i] != Long.MAX_VALUE
							&& values[i] != Long.MIN_VALUE && values[i] < value)
						tmpResult[matchNo++] = i;
				}
			} else {
				// ERROR-PRONE
