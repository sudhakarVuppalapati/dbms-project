 ...
 * types.
 */
public class FloatLeafArrayMap extends LeafArrayMap {
	...
	public FloatLeafArrayMap(int n) {
		keys = new float[n];
		values = new int[n];
	}
	 ...
	 * @return
	 */
	public boolean tryAdd(float key, int value) {
		if (currentSize == 0) {
	 ...
	 * @param pos
	 */
	public void addAtPos(float key, int value, int pos) {
		if (pos < currentSize) {
	 ...
	 */
	public int get(float key, IntPushOperator results) {
		if (currentSize == 0) {
			// continue search
	...
	public int continueGet(int pos, float key, IntPushOperator results) {
		while (pos < currentSize && keys[pos] == key) {
	 ...
	 * @return
	 */
	public int queryRange(float lowKey, float highKey, IntPushOperator results) {
		if (currentSize == 0) {
	...
	public int continueScan(int pos, float highKey, IntPushOperator results) {
