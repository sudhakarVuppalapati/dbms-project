...
public class FirstPhaseTest {
	...
	private static boolean checkTableContents() {
		...
		for (i = 0; i < currentNumberOfTables; i++) {
			...
			try {
				rowOp = myDatabase.getStorageInterface().getTableByName(
						tableList.get(i).tableName).getRows();
	...
	private static boolean checkTables() {
		Operator<Table> tableOp = myDatabase.getStorageInterface().getTables();
	...
	private static boolean checkTableSchemas() {
		...
		for (i = 0; i < currentNumberOfTables; i++) {
			...
			try {
				colOp = myDatabase.getStorageInterface().getTableByName(
						tableList.get(i).tableName).getAllColumns();
