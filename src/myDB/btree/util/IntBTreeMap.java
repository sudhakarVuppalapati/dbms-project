...
public class IntBTreeMap extends BTreeMap {
	...
	public void delete(Object startingKey, Object endingKey)
		...
		try {
			...
			if (lowKey > highKey)
				...
				btree.remove(lowKey, highKey, BTreeConstants.ALL_MAPPINGS);
		} catch (ClassCastException cce) {
	...
	public Operator<Row> rangeQuery(Object startingKey, Object endingKey)
		...
		int lowKey, highKey;
		try {
			lowKey = (startingKey != null) ? ((Integer) startingKey).intValue()
					: Integer.MIN_VALUE;
			highKey = (endingKey != null) ? ((Integer) endingKey).intValue()
					: Integer.MAX_VALUE;
			if (lowKey > highKey)
				throw new InvalidRangeException();
		} catch (ClassCastException cce) {
