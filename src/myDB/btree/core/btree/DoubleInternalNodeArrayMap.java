 ...
 * @author jens
 */
public class DoubleInternalNodeArrayMap extends InternalNodeArrayMap {
	 ...
	 * keys
	 */
	protected DoubleBTreeNode[] nodes;
	...
	public DoubleInternalNodeArrayMap(int n) {
		keys = new double[n];
		nodes = new DoubleBTreeNode[n + 1];

		nodes[0] = myDB.btree.core.btree.DoubleInternalNode.NULL;
	}
	 ...
	 * with entries from (mid+1) to (currentSize-1). The key mid is no doubleer
	 ...
	 * @return
	 */
	public void put(double key, DoubleBTreeNode rightNode) {
		if (currentSize == 0) {
			keys[0] = key;
			nodes[1] = rightNode;
			currentSize++;
	 ...
	 * @return
	 */
	public DoubleBTreeNode get(double key) {
