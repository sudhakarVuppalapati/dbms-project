 ...
 * types.
 */
public class LongLeafArrayMap extends LeafArrayMap {

	protected long[] keys;

	protected int[] values;

	public int getIntervalPosition(long key) {
		if (currentSize == 0) {
			return -1;
		} else {
	 ...
	 * @return
	 */
	public static int binarySearch(long[] a, long key, int from, int to) {
		int low = from;
		int high = to;
	 ...
	 * @return
	 */
	public LongLeafArrayMap split() {
		LongLeafArrayMap newMap = new LongLeafArrayMap(keys.length);
	 ...
	 * @return
	 */
	public boolean tryAdd(long key, int value) {
		if (currentSize == 0) {
	 ...
	 * @param pos
	 */
	public void addAtPos(long key, int value, int pos) {
		if (pos < currentSize) {
	 ...
	 */
	public int get(long key, IntPushOperator results) {
		if (currentSize == 0) {
			// continue search
	...
	public int continueGet(int pos, long key, IntPushOperator results) {
		while (pos < currentSize && keys[pos] == key) {
	 ...
	 * @return
	 */
	public int queryRange(long lowKey, long highKey, IntPushOperator results) {
		if (currentSize == 0) {
	...
	public int continueScan(int pos, long highKey, IntPushOperator results) {
		boolean returnedSomething = false;
		while (pos < currentSize && keys[pos] <= highKey) {
			results.pass(values[pos]);
			pos++;
