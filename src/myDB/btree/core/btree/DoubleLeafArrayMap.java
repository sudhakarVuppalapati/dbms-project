 ...
 * types.
 */
public class DoubleLeafArrayMap extends LeafArrayMap {
	
	protected double[] keys;

	protected int[] values;

	public int getIntervalPosition(double key) {
		if (currentSize == 0) {
			return -1;
		} else {
	 ...
	 * @return
	 */
	public static int binarySearch(double[] a, double key, int from, int to) {
		int low = from;
		int high = to;
	...
	public DoubleLeafArrayMap(int n) {
		keys = new double[n];
		values = new int[n];
	}

	public double getMidKey() {
	 ...
	 * @return
	 */
	public boolean tryAdd(double key, int value) {
		if (currentSize == 0) {
	 ...
	 * @param pos
	 */
	public void addAtPos(double key, int value, int pos) {
		if (pos < currentSize) {
	 ...
	 */
	public int get(double key, IntPushOperator results) {
		if (currentSize == 0) {
			// continue search
	...
	public int continueGet(int pos, double key, IntPushOperator results) {
		while (pos < currentSize && keys[pos] == key) {
	 ...
	 * @return
	 */
	public int queryRange(double lowKey, double highKey, IntPushOperator results) {
		if (currentSize == 0) {
	...
	public int continueScan(int pos, double highKey, IntPushOperator results) {
