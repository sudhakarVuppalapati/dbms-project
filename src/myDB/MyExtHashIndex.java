...
public class MyExtHashIndex implements HashIndex {
	...
	/**
	 * Since Java has no exponential operator, I store the value of 2^gDepth
	 * here, to fast compute when needed
	 */
	private int powerDepth;

	/**
	 * Lists of buckets, of which items are an array of size BUCKET_SIZE. Items
	 * of an array is an array-liked list with key's hashCode at the beginning,
	 * followed by rowIDs in ascending order
	 */
	private List buckets;

	/**
	 * The local depths, and also the implicit directory. To be double when
	 * needed
	 */
	private int[] lDepths;

	/**
	 * List keeping track of the last occupied slot in buckets. If one bucket is
	 * full, its corresponding freeSlot value will be BUCKET_SIZE
	 */
	...
	private static int hash(int key, int depth) {
		return MyUtil.hash32shiftmult(key) & depth;
	}
	 ...
	 */
	public MyExtHashIndex(String indexDes, Type t, Table tableObj, Column attribute) {
		column = attribute;
		type = t;
	...
	private void insert(int key, int rowID) {
		...
		/**
		 * Step 2: Found the bucket. Try to add into the matching bucket. Stop
		 * if successful. Due to the way data entries are organized, we need to
		 * perform binary search over the keys, and within every data entry -
		 * over the list of rowIDs.
		 * 
		 */
		bucket = (Object[]) tmp;
		...
		for (; low <= high;) {
			...
			// Key found, just insert the value into the matching data entry.
				...
				int entrySize = entry[1]; // The second elements are data entry
											// size
				...
				for (; low1 <= high1;) {
					...
					if (midVals < rowID)
						...
						/*
						 * System.out.println(" Couple (" + key + "," + + rowID
						 * + ") is found");
						 */
				...
				/*
				 * System.out.println(" Key" + key + " is found, but rowID " + +
				 * rowID + " is new");
				 */
				...
				if (low1 < entrySize) {
					System.arraycopy(entry, low1, entry, low1 + 1, entrySize
							- low1);
					entry[low1] = rowID;
				} else
					entry[entrySize] = rowID;
				entry[1]++;
				return;
			}
		}
		/**
		 * Step 3: Found the bucket, but key is new, then try to add it into the
		 * bucket.
		 */
		/*
		 ...
		 * " to matching bucket failed. Do extending");
		 */
		Object[] oldBucket = bucket.clone();

		int k = lDepths.length;

		while (true) { // Repeat do extending until every data entries fit
						// within a bucket
			freeSlot[bucketNo] = 0;
			...
			for (i = 0; i < BUCKET_SIZE; i++) {
				...
				if (tmp != null) {
					...
					for (; low1 <= high1;) {
						...
						int tmpKey = entry1[0]; // The beginning elements is key
												// content
						...
						if (tmpKey < oKey)
							...
							if (newSize == entry1.length) {
								int[] newEntry = new int[Math.round(newSize
										* FACTOR)];
								System.arraycopy(entry1, 0, newEntry, 0,
										newSize);
								entry1 = newEntry;
							}
					...
					if (low1 > Integer.MIN_VALUE)
						if (freeSlot[bucketNo] < BUCKET_SIZE) {
							if (low1 < freeSlot[bucketNo]) {
								System.arraycopy(bucket, low1, bucket,
										low1 + 1, freeSlot[bucketNo] - low1);
	...
	/**
	 * Construct the index in bulk loading-liked fashion. Column values are
	 * obtained by calling MyColumn.getDataArrayAsObject()
	 */
		...
		for (depth = powerDepth; depth >= 0; depth = (depth + 1) / 2 - 1) {
			...
			if (tmp != null) {
				...
				for (; low <= high;) {
					...
					int tmpKey = tmpEntry[0]; // The beginning elements is key
												// content
					if (tmpKey < key)
						low = mid + 1;
					else if (tmpKey > key)
						high = mid - 1;
					// Key found, just retrieve the values in entry from 3rd
					// elements on
					else {
						int size = tmpEntry[1];
						int[] newEntry = new int[entry.length + size - 2];
						System.arraycopy(entry, 0, newEntry, 0, entry.length);
						System.arraycopy(tmpEntry, 2, newEntry, entry.length,
								size - 2);
						entry = newEntry;
						break;
					}
		...
		for (depth = powerDepth; depth >= 0; depth = (depth + 1) / 2 - 1) {
			...
			if (tmp != null) {
				...
				for (; low <= high;) {
					int mid = (low + high) >> 1;
					entry = (int[]) bucket[mid];
					int tmpKey = entry[0]; // The beginning element is key
											// content
					if (tmpKey < key)
						low = mid + 1;
					else if (tmpKey > key)
						high = mid - 1;
					// Key found, further search for the matching row and delete
					// if found.
					else {
						boolean found = false;
						int entrySize = entry[1]; // The second element is data
													// entry size
						int low1 = 2, high1 = entrySize - 1;
						for (; low1 <= high1;) {
							int mid1 = (low1 + high1) >> 1;
						...
						if (found) {
							System.arraycopy(entry, low1 + 1, entry, low1,
									entry.length - 1 - low1);
							entry[1]--;
							if (entry[1] == 0) {
								System.arraycopy(bucket, mid + 1, bucket, mid,
										freeSlot[bucketNo] - 1 - mid);
								freeSlot[bucketNo]--;
							}
						}
						return;
			...
			if (tmp != null) {
				...
				for (; low <= high;) {
					...
					if (tmpKey < key)
						...
						high = mid - 1;
					// Key found, further search for the matching oldRow and
					// delete
					else {
						boolean found = false;
						int entrySize = entry[1]; // The second element is data
						...
						if (found) {
							...
							for (; low1 <= high1;) {
								...
								if (midVals < newRowID)
									...
									return; // new row found, dont need to
											// insert
							}
							if (low1 < entrySize) {
								System.arraycopy(entry, low1, entry, low1 + 1,
										entrySize - low1);
								entry[low1] = newRowID;
							} else
								entry[entrySize] = newRowID;
							entry[1]++;
						}
						return;
					}
