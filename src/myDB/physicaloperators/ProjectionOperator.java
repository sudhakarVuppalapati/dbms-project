 ...
 * @author razvan
 * 
 *         implementation of the Projection relational operator by different
...
public class ProjectionOperator {
	 ...
	 * 
	 * @param input
	 *            - The relation passed/pipelined as input(from previous
	 *            operators)
	 * @param prjAtrributes
	 *            - The projection attributes
	 * @return - the result of projecting all the tuples of the input(relation),
	 *         after duplicate elimination
	 * 
	 *         This method implements the most basic strategy of projection,
	 *         i.e.: scanning,sorting,eliminating duplicates
	 */
	public static Map<String, Column> project(Map<String, Column> input,
			String[] prjAttributes) throws NoSuchColumnException {
		...
		Map<Column, Object> tmpArrContainer = new HashMap<Column, Object>(); // aux
																				// var
																				// for
																				// holding
																				// the
		...
		/*
		 * iterate through the set of columns and put into the result only the
		 * columnswhose names appear in the list of projection attributes (and
		 * also sort the column)
		 */
		for (int i = 0; i < prjAttributes.length; i++) {
			c = input.get(prjAttributes[i]);
			if (c == null) { // the input relation doesn't contain
								// prjAttributes[i] column
		...
		// By this point the all the columns are sorted independently which is
		// wrong!!!!
	...
	/**
	 * This implementation doesn't eliminate duplicates ... which might also be
	 * a kind of projection
	 */
	public static Map<String, Column> projectWithDuplicates(
			Map<String, Column> input, String[] prjAttributes)
		...
		// verify if the projection is possible,i.e.: all the projection
		// attributes are in the input table
		for (int i = 0; i < prjAttributes.length; i++) {
			c = input.get(prjAttributes[i]);
			if (c == null) { // the input relation doesn't contain
								// prjAttributes[i] column
